...
$sql2 = "SELECT * FROM approval WHERE dep_id=".$dep."
				...
				$msg.='<p>&nbsp</p>';
				$msg.='<p>There is a loan application From <B>'.$name.'</B>  and is pending your approval.  </> ';
				 $msg.='<p>&nbsp</p>';
        ...
        $msg = '<p>Dear Sir/Madam ,</p>';
		$msg.='<p>&nbsp</p>';
        $msg.='<p>There is a loan application From <B>'.$name.'</B>  and is pending to your approval.  </> ';
         $msg.='<p>&nbsp</p>';
        $msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
		 $msg.='<p>&nbsp</p>';
		$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
		...
		$msg.= "<p><a href='http://myigen.com:8081/HR/igen'>Click here </a>, To login to ".$c_name." @WORK  </p>";
		 ...
		 mailto($email2, $subject, $msg, $headers);
					...
					$status="Approved";
						   ...
						   $msg = '<p>Dear All , </>';
							   $msg.='<p>&nbsp</p>';
								$msg.='<p>The loan application From <B>  '.$name.'</B> has been approved by <B>'.$full_name.'</B>.</P>' ;
								  $msg.='<p>&nbsp</p>';
								$msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
								 $msg.='<p>&nbsp</p>';
								$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
...
$sql = "SELECT * FROM employee_loan WHERE id = " . $id;
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);
	...
	$msg = '<p>Dear ' . $name . ',</p>';
	 $msg.='<p>&nbsp</p>';
        $msg.='<p>Your loan application has been approved by<B> '.$full_name.'</B>.</p>' ;
        $msg.='<p>&nbsp</p>';
        $msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
		 $msg.='<p>&nbsp</p>';
		$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
...
if ($status == "Rejected") {
 $msg = '<p>Dear ' . $name . ',</p>';
	 $msg.='<p>&nbsp</p>';
        $msg.='<p>Your loan application has been rejected by<B> '.$full_name.'</B>.</p>' ;
        $msg.='<p>&nbsp</p>';
        $msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
		 $msg.='<p>&nbsp</p>';
		$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
		...
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n"; 
         mailto($email, $subject, $msg, $headers);
    if ($aid1 == 1) {
        $sql2 = "UPDATE employee_loan SET approval_1='" . $status . "' where id='" . $id . "'";
    } elseif ($aid2 == 2) {
        $sql2 = "UPDATE employee_loan SET approval_2='" . $status . "' where id='" . $id . "'";
    }
    $sql1 = "UPDATE employee_loan SET loan_status='" . $status . "' where id='" . $id . "'";
} else {
    if ($aid1 == 1) {
    ...
    if ($aid3 == 3) {
        $app[] = "loan_status='" . $status . "'";
    }
    for ($i = 0; $i < count($app); $i++) {
        $approve = $approve . $app[$i] . ',';
    }
    $approve = substr($approve, 0, -1);
    $sql1 = "UPDATE employee_loan SET " . $approve . " where id='" . $id . "'";
}

if ($status == "Rejected" || $aid3 == 3) {
//        app_mail_process($emp_id, 'loan', $id, $status);
}
 $sql1 = "UPDATE employee_loan SET loan_status='" . $status . "' where id='" . $id . "'";
