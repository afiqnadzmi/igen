...
// Update Employee Claim
$sql = 'UPDATE employee_claim SET claim_status = "' . $status . '", status2 = "' . $status2 . '" WHERE id = ' . $id . ';';
...
if ($status == "Approved"){
	 $msg = 'Dear ' . $name . ',<br>';
		...
		if ($status == "Rejected") {
	    $msg = 'Dear ' . $name . ',<br>';
        $msg.='<p>Your Claim application has been Rejected by<B>  '.$full_name.'</B>. </p>' ;
		$msg.='<p>&nbsp</p>';
