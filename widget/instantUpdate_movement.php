...
$sql_3 = "SELECT * FROM employee_movement WHERE id=" . $id ;
...
$sql = 'UPDATE employee_movement SET request_status	= "' . $status . '" , status2 = "' . $status2 . '" WHERE id = ' . $id . ';';
...
if ($status == "Approved"){
		...
		if ($status == "Rejected") {
				...
				$msg = '<p>Dear ' . $name . ',</p>';
				$msg.='<p>Your Movement application has been Rejected by  </B><B>'.$full_name.'</B>. </p>' ;
				$msg.='<p>&nbsp</p>';
