...
if (move_uploaded_file($_FILES['excel_file']['tmp_name'], $new_path)) {
    parseExcel($new_path);
    header("location:?loc=import_epf&m=done");
} else {
    header("location:?loc=import_epf&m=error");
}
...
function parseExcel($excel_file_name_with_path) {
    ...
    for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
        if ($data->sheets[0]['cells'][$i][2] != "") {
            ...
            $rate = $data->sheets[0]['cells'][$i][3];
            $employer_rate = $data->sheets[0]['cells'][$i][4];
            $deduct_rate = $data->sheets[0]['cells'][$i][5];

            $sql = "update employee_epf set rate=" . $rate . ",employer_rate=" . $employer_rate . ",deduct_rate=" . $deduct_rate . " where fr_amt=" . $fr_amt . " and to_amt=" . $to_amt;
            mysql_query($sql);
        }
    }
}
