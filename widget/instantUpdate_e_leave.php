...
$from_date=$row_3['from_date'];
$to_date=$row_3['to_date'];
...
//notify Selected Staff
 $sql_no = "SELECT e.full_name, e.email, n.id, n.emp_id from employee e, notify n  WHERE n.dep_id='" . $dep . "'AND n.emp_id=e.id";
 $rs_no= mysql_query($sql_no);
//work flow approval
...
if ($leavebal >= 0) {
    ...
    $sql = 'UPDATE employee_leave SET request_status = "' . $status . '", status2 = "' . $status2 . '", reject_reason = "' . $request_status . '" WHERE id = ' . $id . ';';
   ...
   if ($status == "Approved" || $status == "Rejected" || $status == "Pending"){
	...
	//Senind email notification to the leave applicant
	if ($status == "Approved"){
	/*
	 $msg = '<P>Dear ' . $name . ',</p>';
...
*/		
		 //notify selected staff
		   ...
		   while ($row = mysql_fetch_array($rs_no)) {
	
                          /*            
                       $msg = '<p>Dear ' . ucfirst($row['full_name']) . ',</p>';
					 $msg.='<p>&nbsp</p>';
					$msg.='<p>For your information, <B>'.$name.'</B> will be on leave from <B>'.$from_date.'</B> to <B>'. $to_date.'</B>.</p>' ;
					 $msg.='<p>&nbsp</p>';
					$msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
					 $msg.='<p>&nbsp</p>';
					$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
					...
					$msg.= "<p><a  href='http://myigen.com:8081/HR/igen/'>Click here </a>, To login to ".$c_name." @WORK  </p>";
					 $msg.='<p>&nbsp</p>';
        			$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n"; 
                     // mailto($row['email'], $subject, $msg, $headers);					  
                          */              
                                    }
