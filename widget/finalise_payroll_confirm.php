...
while ($row = mysql_fetch_array($rs)) {
	...
	$sql_shift = "SELECT * FROM shift where id=".$shift_id;
    $rs_shift = mysql_query($sql_shift);
	$row_shift = mysql_fetch_array($rs_shift);
	$total_working_hours=$row_shift['total_hours'];
    $total_working_day = days_in_month($emp_id, $month, $year);

    $dep_id = $row['dep_id'];
	$branch_id = $row['branch_id']; 
    $emp_name = $row['full_name'];
    $get_days_at_work = days_at_work($emp_id, $month, $year, $total_working_hours);
    $get_days_absent = days_absent($emp_id, $month, $year, $total_working_hours);
    $get_basic = basic_salary($emp_id);
    $get_overtime = overtime($emp_id, $month, $year, $total_working_hours);
    $get_designation = designation($emp_id);
	$sql_inc = 'SELECT MONTH(incre_date) as month, Year(incre_date) as year,  increment_by FROM simulation_inrement WHERE emp_id = ' . $emp_id . ' limit 1 ';
    $rs_inc = mysql_query($sql_inc);
    $row_inc = mysql_fetch_array($rs_inc);
    $incre_month=$row_inc['month'];
	$increment_b=$row_inc['increment_by'];
    ...
    $get_salary_per_hours = salary_per_hours($get_basic, $emp_id, $month, $year, $total_working_hours);
    ...
    $get_unpaid_leave_time = unpaid_leave_time($emp_id, $month, $year,  $total_working_hours);
    ...
    $total_allowance = allowance($emp_id, $month, $year); 
	$total_deduction=deduction($emp_id, $month, $year); 


    $get_unpaid_leave = unpaid_leave($emp_id, $month, $year, $get_salary_per_hours, $total_working_hours);

    $get_unpaid_leave_days = $get_unpaid_leave_time / $total_working_hours;
    $ot_dur = ot_duration($emp_id, $month, $year);
    $get_loan = loan($emp_id);
    $get_absent = absent($emp_id, $month, $year, $get_salary_per_hours, $total_working_hours);
...
//if employee join in that particular month
    ...
    if ($resign_date == false) {
        $gross_pay = $get_basic + $get_overtime + $get_bonus - $get_unpaid_leave - $get_absent - $minusJoin; // - $get_late_leave //Removed from the calculation as we are not calcualting the late leave
    } else {
        $ori_gross_pay = $get_basic;
        $resigner_no_work_days = days_in_month($emp_id, $month, $year, date('j', strtotime($resign_date))) - days_absent($emp_id, $month, $year, $total_working_hours);
        ...
        $get_days_at_work = $resigner_no_work_days - $get_paid_leave - ($get_unpaid_leave_time / $total_working_hours);
    }


    $get_socso = socso($gross_pay + $get_allowance_socso, $emp_id);
    $get_epf = epf($emp_id, $gross_pay + $get_allowance_epf);
    $get_pcb = pcb($emp_id, $gross_pay + $get_allowance_pcb, $get_epf);
	$get_eis =eis(gross_pay, $emp_id);

    if ($get_bonus == 0) {
        $get_employer_epf = employer_epf($emp_id, $gross_pay + $get_allowance_epf);
    } else {
        $get_employer_epf = employer_epf_bonus($emp_id, $gross_pay + $get_allowance_epf, $get_basic, $get_bonus);
    }


    $gross_pay = $gross_pay + $total_allowance;
    $zakat = "";
    if (is_muslim($emp_id)) {
    ...
    if (chk_age_more_55($emp_id)) {
        $get_employer_socso = employer_socso2($gross_pay + $get_allowance_socso, $emp_id); //more than 55 years old
       // $get_socso = 0; //more than 55 years old no need pay socso
    } else {
        $get_employer_socso = employer_socso($gross_pay + $get_allowance_socso, $emp_id);
    }
	$get_employer_eis =0;
	$get_employee_eis=0;
	$get_employer_fweis=0;
	if(trim($nationality)=="Malaysia"){
			$get_employer_eis = employer_eis($gross_pay, $emp_id);
			$get_employee_eis = employee_eis($gross_pay, $emp_id);
		}else{
			$get_employer_fweis = employer_fweis($gross_pay, $emp_id);
			$get_employer_hrdf = employer_hrdf($gross_pay, $emp_id);
		}


    $net_pay = $gross_pay - $get_epf - $get_socso - $get_pcb - $get_employee_eis - $zakat + $get_claim - $get_advance_salary - $get_loan - $total_deduction;
	...
	$arr[] = insertkeyvalue("eis", $get_eis);
    ...
    $arr[] = insertkeyvalue("late_leave_time", $get_late_leave_time);
    $arr[] = insertkeyvalue("dep_id", $dep_id);
	$arr[] = insertkeyvalue("branch_id", $branch_id);
    $arr[] = insertkeyvalue("employer_epf", $get_employer_epf); 
    $arr[] = insertkeyvalue("employer_socso", $get_employer_socso);
	$arr[] = insertkeyvalue("employer_eis", $get_employer_eis);
	$arr[] = insertkeyvalue("employee_eis", $get_employee_eis);
	$arr[] = insertkeyvalue("fweis", $get_employer_fweis);
	$arr[] = insertkeyvalue("hrdf", $get_employer_hrdf);
	$arr[] = insertkeyvalue("deduction", $total_deduction);
    insert2($table, $arr);
    unset($arr);
    ...
    // Advanced salary Payroll Processed
	...
	//Claim Payroll Processed
	$sql_claim = 'insert into payroll_claim(claim_amount, claim_type, emp_id, request_date,payout_month,payout_year,payroll_finalised_id) 
				  select amount,claim_item_title,emp_id,claim_date,'.$month.','.$year.','. $new_id .' from employee_claim 
				  where emp_id ="' . $emp_id. '"  and year(claim_date)=' . $year . ' and month(claim_date)=' . $month .' and claim_status="Approved"';
    $query_claim = mysql_query($sql_claim);
	
	//Fixed Deduction Payroll Processed
	$sql_fd = 'insert into payroll_deduction(deduction_amount, deduction_title, type, emp_id,payout_month,payout_year,payroll_finalised_id) 
				SELECT ed.deduction_amount, dp.deduction_name, ed.type, ed.emp_id ,'.$month.','.$year.','. $new_id .' FROM deduction_payroll dp 
				INNER JOIN employee_deduction ed on dp.id = ed.deduction_id 
				WHERE ed.emp_id="'.$emp_id.'" and type="1"';
    $query_fd = mysql_query($sql_fd);
	
	//Evolved Deduction Payroll Processed
	$sql_ed = 'insert into payroll_deduction(deduction_amount, deduction_title,type, emp_id,payout_month,payout_year,payroll_finalised_id) 
				SELECT ed.deduction_amount, dp.deduction_name, ed.type, ed.emp_id ,'.$month.','.$year.','. $new_id .' FROM deduction_payroll dp 
				INNER JOIN employee_deduction ed on dp.id = ed.deduction_id 
				WHERE ed.emp_id="'.$emp_id.'" AND ed.month="'.$month.'" AND ed.year="'.$year.'"and type="2"';
    $query_ed = mysql_query($sql_ed);
