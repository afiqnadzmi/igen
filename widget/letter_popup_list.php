...
<div style="border: 1px solid black;">
    <table id="emp_table" style="width: 100%; border-collapse: collapse;">
        ...
        if (isset($_GET["emp_name"]) == true) {
            $sql = 'SELECT e.id, e.full_name, p.position_name, e.salary, e.address, e.email FROM employee AS e INNER JOIN position AS p ON p.id = e.position_id WHERE e.full_name LIKE "%' . $_GET["emp_name"] . '%" AND e.company_id=' . $_GET["c"];
        } else {
            $sql = 'SELECT e.id, e.full_name, p.position_name, e.salary, e.address , e.email FROM employee AS e INNER JOIN position AS p ON p.id = e.position_id WHERE e.company_id=' . $_GET["c"];
        }
        ...
        while ($row = mysql_fetch_array($query)) {
            ...
            ?>
            <td><a onclick="select_emp('<?php echo $_GET["id"]; ?>','<?php echo $row["full_name"]; ?>', '<?php echo $row["position_name"] ?>', '<?php echo $row["salary"]; ?>', '<?php echo $row["email"]; ?>','<?php echo $row["address"] ?>', '<?php echo $_GET["type"]; ?>')">Select</a></td>
            <?php
            echo '</tr>';
        }
        ?>
...
<script type="text/javascript">
    ...
    function select_emp(id, emp_name, position, salary, email, address, type){
