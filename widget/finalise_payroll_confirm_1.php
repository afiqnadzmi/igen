...
while ($row = mysql_fetch_array($rs)) {
	...
	//manual ot calculation
	 $get_overtime=ot_deduction($emp_id, $month, $year);
   ...
   $get_at_deduction=at_deduction($emp_id, $month, $year);
   $get_per_deduction=per_deduction($emp_id, $month, $year);
...
//if employee join in that particular month
	...
	$arr[] = insertkeyvalue("at_deduction", $get_at_deduction);
	$arr[] = insertkeyvalue("per_deduction", $get_per_deduction);
    insert2($table, $arr);
    unset($arr);
