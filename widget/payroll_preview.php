      ...
      <div> <p style="text-align: center;font-size:large;"><b>Payroll Summary</b></p> </div>
        ...
        <div class="tablediv">
            ...
            <?php
                if (isset($_GET['emp_id']) == true && isset($_GET["mon"]) && isset($_GET["year"])) {
					...
					$total_pcb="0.00";$total_zakat="0.00";$get_total_loan ="0.00";$total_net_pay="0.00";$deduction="0.00";$total_fweis="0.00";
					$total_absent="0.00";$total_hrdf="0.00";$total_absent_days="0";
                   ...
                   while($rowEmpStatus = mysql_fetch_array($rs)){
                    ...
                    if ($emp_status == "Active") {
                           ...
                           // include 'app/calculate_payroll.php';
							...
							$get_allowance_epf = allowance($emp_id,$month, $yea, "epf");
							$get_allowance_socso = allowance($emp_id, $month, $yea, "socso");
							$get_allowance_pcb = allowance($emp_id, $month, $yea, "pcb");
							$total_allowance = allowance($emp_id, $month, $yea);
							...
							//echo "$get_absent = absent($emp_id, $month, $year, $get_salary_per_hours)";
							 ...
							 //$get_employer_fweis = employer_fweis($gross_pay + $total_allowance, $emp_id);
							 if(trim($nationality)=="Malaysia"){
								$get_employer_eis = employer_eis($gross_pay + $total_allowance, $emp_id);
								$get_employee_eis = employee_eis($gross_pay + $total_allowance, $emp_id);
							}else{
								$get_employer_fweis = employer_fweis($gross_pay + $total_allowance, $emp_id);
								$get_employer_hrdf = employer_hrdf($gross_pay + $total_allowance, $emp_id);
							}
							...
							$gross_pay = $gross_pay + $total_allowance + $get_comission_epf - $get_employee_eis;
							if (is_muslim($emp_id)) {
								...
								$net_pay = $gross_pay - $get_epf - $get_socso - $pcb + $get_claim - $get_advance_salary - $get_loan - $total_deduction;
								//echo "$net_pay = $gross_pay - $get_epf - $get_socso - $pcb + $get_claim";
							} else {
								//echo "$gross_pay - $get_epf - $get_socso - $get_pcb + $get_claim";
								$net_pay = $gross_pay - $get_epf - $get_socso - $get_pcb + $get_claim - $get_advance_salary - $get_loan - $total_deduction;
               ...
               // }
				...
				if ($finalised==true) {
					...
					//Deduction
					//$sql_deduction = 'SELECT * FROM  employee_deduction ed INNER JOIN deduction_payroll dp on dp.id=ed.deduction_id  where emp_id = "' . $emp_id . '" and 	((month(month)="' . $month . '" and year(year)="' .$year.'") or type=1)';
					$sql_deduction = 'SELECT ed.deduction_amount as edamount, dp.deduction_name FROM employee_deduction ed INNER JOIN deduction_payroll dp on dp.id=ed.deduction_id where ed.emp_id ="' . $emp_id . '"  and ((ed.month="' . $month . '" and ed.year="' .$year.'") or type=1)';
					$rs_deduction = mysql_query($sql_deduction);
					$num_row_deduction = mysql_num_rows($rs_deduction);	
				}
			...
			$total_ee_epf += $get_epf; $total_ee_socso += $get_socso; $total_er_epf += $get_employer_epf;$total_er_socso += $get_employer_socso;
			$total_pcb += $get_pcb; $total_zakat += $zakat; $get_total_loan +=$get_loan; $total_net_pay += $net_pay; $total_absent+=$get_absent; $total_absent_days+=$get_days_absent;
			$total_er_eis += $get_employer_eis;$total_ee_eis += $get_employee_eis;$total_fweis+=$get_employer_fweis;$total_hrdf+=$get_employer_hrdf;
			/*if(trim($nationality)=="Malaysia"){
				
			}else{
				
				$get_employer_eis="0.00";
				$get_employee_eis="0.00";
             ...
             <tr>
				...
				if($num_row > 0){
					while ($row = mysql_fetch_array($rs_claim)) {
						$s_arr = explode(" ", $row['claim_item_title']);
						//echo "s".ucfirst($s_arr['0'])."_".$s_arr['1'];
						echo "<table class='mid-table' style='border-collapse: collapse;width: 100%;'><tr><td  style='width: 146px;'>".ucfirst($s_arr['0'])."<span style='color:#F8F8F8'>_</span>".$s_arr['1']."</td><td>:</td><td>".number_format($row['amount'], 2)."</td></tr></table>"; 
				?>	
						
				<?php
                      $total_claim += $row['amount'];
					}
				}else{
					echo"0.00";
				}
				...
				if($num_row_deduction > 0){
					while ($row = mysql_fetch_array($rs_deduction)) {
						$s_arr = explode(" ", $row['deduction_name']);
						//echo "s".ucfirst($s_arr['0'])."_".$s_arr['1'];
						echo "<table  class='mid-table' style='border-collapse: collapse;width: 100%;'><tr><td  style='width: 116px;'>".ucfirst($s_arr['0'])."<span style='color:#F8F8F8'>_</span>".$s_arr['1']."</td><td>:</td><td>".number_format($row['edamount'], 2)."</td></tr></table>"; 
				?>						
				<?php
                      $deduction += $row['edamount'];
					}
				}else{
					echo"0.00";
				}
				...
				<td><?php echo number_format($get_absent, 2); ?></td> 
				<td><?php echo $get_days_absent; ?></td> 				
				...
				<td><?php echo number_format($get_employer_fweis, 2); ?></td>
				<td><?php echo number_format($get_employer_hrdf, 2); ?></td>				
		...
		?>
			<tr class="sub-total">
				...
				<td><?php echo number_format($total_absent, 2); ?></td>	
				...
				<td><?php echo number_format($total_fweis, 2); ?></td>	
				<td><?php echo number_format($total_hrdf, 2); ?></td>					
