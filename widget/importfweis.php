...
if (move_uploaded_file($_FILES['excel_file']['tmp_name'], $new_path)) {
    parseExcel($new_path);
    header("location:?loc=import_fweis&m=done");
} else {
    header("location:?loc=import_fweis&m=error");
}

function parseExcel($excel_file_name_with_path) {
    $d_sql = 'delete from employee_fweis';
