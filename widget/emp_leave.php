...
?>
<div class="rounded_corners" id="choosetemplate" style='position:absolute;z-index:1000;top:160px;background-color:white;border:10px solid #C4C4C7;margin-left:150px;width:520px;height:325px;'>
    <div style="padding:5px;width:490px;color:black;height:25px;">
        <span style='float:left;font-size:13px;font-weight:bold;width:220px'>Employee's Leave on <?php echo $this_date ?></span>
    ...
    <div style='overflow:auto;width:506px;height:300px;'>
        <?php
        $sql = "SELECT e.id,el.num_days, e.full_name as fn, from_date, to_date,DATEDIFF(to_date,from_date) as dd FROM employee_leave el
                join employee e on el.emp_id=e.id
		where request_status='Approved' and (from_date='" . $this_date . "' or to_date='" . $this_date . "' or (from_date<'" . $this_date
                . "' and to_date>'" . $this_date . "'))" . $sqlAdd;
        $rs = mysql_query($sql);
        if ($rs && mysql_num_rows($rs) > 0) {
            ...
            echo "No employee apply leave on " . $this_date;
        }
        ?>
