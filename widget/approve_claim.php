				...
				$user_id=$superv1;
                   ...
                   $rs4 = mysql_query($sql4);
					...
					$status="Approved";
		                    ...
		                    $msg = '<p>Dear All , </p>';
						    $msg.='<p>&nbsp</p>';
					        $msg.='<p>The Claim application From  <B>'.$name.'</B> has been approved by <B>'.$full_name.'</B>.</p>' ;
					        $msg.='<p>&nbsp</p>';
		...
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n"; 
		 ...
		 mailto($email2, $subject, $msg, $headers);
		  mailto($email3, $subject, $msg, $headers);
				...
				$msg.='<p>&nbsp</p>';
        $msg.='<p>There is a Claim application From <B>'.$name.'</B>  and is pending to your approval.  </p> ';
		$msg.='<p>&nbsp</p>';
        $msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
		 $msg.='<p>&nbsp</p>';
		$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
...
$sql = "SELECT * FROM employee_claim WHERE id = " . $id;
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);
if ($status == "Approved"){
	      $msg = '<P>Dear ' . $name . ',</p>';
        $msg.='<p>&nbsp</p>';
        $msg.='<P>Your claim application has been approved by <B>'.$full_name.'</B>.</P>' ;
		 $msg.='<p>&nbsp</p>';
        $msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
		 $msg.='<p>&nbsp</p>';
		$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
...
if ($status == "Rejected") {
        $msg = '<P>Dear ' . $name . ', </P>';
		$msg.='<p>&nbsp</p>';
        $msg.='<P>Your claim application has been Rejected by</P><B> '.$full_name.'</B>. <br>' ;
        $msg.='<p>&nbsp</p>';
       ...
       mailto($email, $subject, $msg, $headers);
    if ($aid1 == 1) {
        $sql2 = "UPDATE employee_claim SET approval_1='" . $status . "' where id='" . $id . "'";
    } elseif ($aid2 == 2) {
        $sql2 = "UPDATE employee_claim SET approval_2='" . $status . "' where id='" . $id . "'";
    }
    $sql1 = "UPDATE employee_claim SET claim_status='" . $status . "' where id='" . $id . "'";
} else {
    if ($aid1 == 1) {
    ...
    if ($aid3 == 3) {
        $app[] = "claim_status='" . $status . "'";
    }
    for ($i = 0; $i < count($app); $i++) {
        $approve = $approve . $app[$i] . ',';
    }
    $approve = substr($approve, 0, -1);
    $sql1 = "UPDATE employee_claim SET " . $approve . " where id='" . $id . "'";
}

if ($status == "Rejected" || $aid3 == 3) {
//        app_mail_process($emp_id, 'claim', $id, $status);
}
$sql1 = "UPDATE employee_claim SET claim_status='" . $status . "' where id='" . $id . "'";
