...
$sql_3 = "SELECT * FROM employee_training WHERE
                            id=" . $id ;
...
$sql2 = "SELECT * FROM approval WHERE dep_id=".$dep."
        		...
        		$msg = '<P>Dear Sir/Madam ,</P>';
						$msg.='<p>&nbsp</p>';
						$msg.='<p><B>'.$name.'</B> has applied Training and is Pending your approval.</p>' ;
						 $msg.='<p>&nbsp</p>';
		 ...
		 mailto($email1, $subject, $msg, $headers);
		    ...
		    mailto($email3, $subject, $msg, $headers);
					...
					$status="Approved";
						...
						$msg = '<p>Dear All , </p>';
						 $msg.='<p>&nbsp</p>';
						$msg.='<p>The  Training application From <B> '.$name.'</B> has been approved by <B>'.$full_name.'</B>.</P>' ;
						$msg.='<p>&nbsp</p>';
						$msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
						 $msg.='<p>&nbsp</p>';
						$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
...
$sql = "SELECT * FROM employee_training WHERE id = " . $id;
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);

if ($status == "Rejected") {
    if ($aid1 == 1) {
        $sql2 = "UPDATE employee_training SET approval_1='" . $status . "' where id='" . $id . "'";
    } elseif ($aid2 == 2) {
        $sql2 = "UPDATE employee_training SET approval_2='" . $status . "' where id='" . $id . "'";
    }
    $sql1 = "UPDATE employee_training SET request_status='" . $status . "' where id='" . $id . "'";
} else {
    if ($aid1 == 1) {
    ...
    $sql1 = "UPDATE employee_training SET " . $approve . " where id='" . $id . "'";
}

if ($status == "Rejected" || $aid3 == 3) {
//        app_mail_process($emp_id, 'training', $id, $status);
}
if ($status == "Approved"){
	 $msg = '<P>Dear ' . $name . ',</p>';
	 
        $msg.='<p>&nbsp</p>';
        $msg.='<P>Your Training  application has been approved by <B>'.$full_name.'</B>.</P>' ;
		 $msg.='<p>&nbsp</p>';
        $msg.='<p><i>*Do not reply to this email as this is a computer generated email</i>.</p>'; 
		 $msg.='<p>&nbsp</p>';
		$msg.='<p><B>Sent By '.$c_name.'  HR Dept</B>.</br></br></p>' ;
		...
		if ($status == "Rejected") {
	     ...
	     $msg = '<P>Dear ' . $name . ', </P>';
		$msg.='<p>&nbsp</p>';
        $msg.='<P>Your Training application has been Rejected by</P><B> '.$full_name.'</B>. <br>' ;
        $msg.='<p>&nbsp</p>';
