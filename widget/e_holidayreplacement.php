...
$replacement =htmlentities($_POST['replacement'], ENT_QUOTES, "UTF-8");
$pending = "Pending";

$sql2 = "INSERT INTO holiday_replacement (replace_date,pub_holiday_id,emp_id, depid, replacement_status,insert_date) 
		VALUES ('" .date('Y-m-d', strtotime($replacement)). "','" . $selectdate . "','" . $emp_id . "','" . $depid. "','" . $pending . "','" . date('Y-m-d') . "');";
   ...
   $subject = 'HR Application';
    if ($rs1 && mysql_num_rows($rs1) > 0) {
			...
			//Sending email
				...
				$msg.='<p>&nbsp</p>';
				$msg.='<p><B>'.$name.'</B> has applied for Public holiday Replacement and is Pending your approval.</p>' ;
				$msg.='<p>&nbsp</p>';
