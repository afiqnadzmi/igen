    ...
    <div class="header_text">
        ...
        <span style="float: right;">
            ...
            if ($igen_a_ea == "a_ea_edit") {
                ...
                ?>
                <table>
                    <tr><td><input class="button" type="button" value ="<?php echo $val ?>" onclick="editClaim('<?php echo $_GET['str1']; ?>','<?php echo $_GET['str2']; ?>','<?php echo $_GET['str3']; ?>')" id="editBut" style="width: 70px;"/></td></tr>
                </table>                
            <?php } ?>
        </span>
    </div>
                  ...
                  while($row2 = mysql_fetch_array($rs2)){
                    ...
                    if (isset($_GET["from"]) == true && isset($_GET["to"]) == true) {
                        $sql = "SELECT * FROM employee as emp
                                ...
                                WHERE emp_c.depid='".$dep."' AND emp_c.request_status IN ('" . $_GET['str1'] . "', '" . $_GET['str2'] . "', '" . $_GET['str3'] . "')
                        ...
                        $sql = "SELECT * FROM employee as emp
                                ...
                                on emp.id = emp_c.emp_id
                                WHERE emp_c.depid='".$dep."' AND emp_c.request_status IN ('" . $_GET['str1'] . "', '" . $_GET['str2'] . "', '" . $_GET['str3'] . "')
                                AND DATE_SUB(CURDATE(),INTERVAL 60 DAY) <= emp_c.leave_date";
                    }
                    if ($igen_companylist == "") {
                        $sql.="";
                    } else {
					...
					if (isset($_GET["from"]) == true && isset($_GET["to"]) == true) {
                        $sql = "SELECT * FROM employee as emp
                                inner join employee_movement as emp_c
                                on emp.id = emp_c.emp_id
                                WHERE emp_c.depid='".$dep."' AND emp_c.request_status IN ('" . $_GET['str2'] . "','" . $_GET['str3'] . "' , 'Approved_lv1')
					...
					if (isset($_GET["from"]) == true && isset($_GET["to"]) == true) {
                        $sql = "SELECT * FROM employee as emp
                                inner join employee_movement as emp_c
                                on emp.id = emp_c.emp_id
                                WHERE emp_c.depid='".$dep."' AND emp_c.request_status IN ('" . $_GET['str2'] . "','" . $_GET['str3'] . "' , 'Approved_lv2')
                                AND emp_c.leave_date BETWEEN '" . $_GET["from"] . "' AND '" . $_GET["to"] . "'";
                    } else {
                        $sql = "SELECT * FROM employee as emp
                                inner join employee_movement as emp_c
				...
				/*
                    ...
                    while ($row = mysql_fetch_array($rs)) {
                        ...
                        echo '<tr class="plugintr">
                            ...
                            <td class="aligncentertable"><a href="images/claim_img/' . $img_path . '" target="_blank">View</a></td>
                            <td>' . $insert_date . '</td>';
                        if (isset($_GET["e"]) && !empty($_GET["e"])) {
...
<script type="text/javascript">
    ...
    function checking(){
        ...
        if(from != "" && to != ""){
            ...
            window.location="?loc=emp_claim&str1="+str1+"&str2="+str2+"&str3="+str3;
        }
    }
    function emp_app(id){ 
