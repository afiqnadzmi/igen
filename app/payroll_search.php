...
if($_GET['dept']!=0){
	...
	$payroll_sql = "SELECT * FROM payroll_report WHERE  dep_id='".$_GET['dept']."' AND branch_id='".$_GET['branch_id']."' AND  MONTH(from_date)='".$_GET['mon']."'  AND YEAR(from_date)='".$_GET['year']."'";
   
}else{
	$payroll_sql = "SELECT * FROM payroll_report WHERE  branch_id='".$_GET['branch_id']."' AND  MONTH(from_date)='".$_GET['mon']."'  AND YEAR(from_date)='".$_GET['year']."'";
}
...
if (isset($_GET["selection"]) == true && $_GET["selection"] == "all") {
    if ($numCheck > 0) {
		...
		if($_POST['dept']!=0){
				$payroll_sql = "SELECT * FROM payroll_report WHERE  dep_id='".$_GET['dept']."' AND branch_id='".$_GET['branch']."' AND  MONTH(from_date)='".$_GET['mon']."'  AND YEAR(from_date)='".$_GET['year']."'";		   
			}else{
				$payroll_sql = "SELECT * FROM payroll_report WHERE  branch_id='".$_GET['branch']."' AND  MONTH(from_date)='".$_GET['mon']."'  AND YEAR(from_date)='".$_GET['year']."'";
			}
		$payroll_rs=mysql_query($payroll_sql);
		$num_rows = mysql_num_rows($payroll_rs);
...
<table>
    <tr>
        ...
        <td>
            ...
            echo '<input id="printpay" style="width: 75px; ' . $print_display . '" type="button" value="Preview123" class="button" ';
			
            if (isset($_GET["selection"]) == true && $_GET["selection"] == "all") {
                echo 'onclick="printpayall(' . $_GET["mon"] . ',' . $_GET["year"] . ',' . $_GET["branch"] . ', ' . $_GET["emp_id"] . ',' . $_GET["dep"] . ')"';
		...
		<td>
            <?php
			if($num_rows==0){
			   echo '<input id="printpay" style="width: 75px;" type="button" value="Print" class="button" onclick="printpreview()">';
			}
			//echo '<input id="printpay" style="width: 75px; ' . $print_display . '" type="button" value="Print" class="button" onclick="printpreview()"';
			//echo 'onclick="printpreview(' . $_GET["mon"] . ',' . $_GET["year"] . ',' . $_GET["branch"] . ', ' . $_GET["emp_id"] . ',' . $_GET["dep"] . ')"';
            ?>
        </td>
    </tr>
</table>
<table> 
    <tr>
        <td style="width: 150px;">Choose One <span class="red"> *</span></td>
        <td>
            <select id="selection" onchange="displaytable(this.value)" style="width: 250px;">
			<?php if($_GET['emp_id']!=true){ ?>
    ...
    <tr>
        <td style="width: 150px;">Department<span class="red"> *</span></td>
        <td>
    ...
    <tr>
        <td style="width: 150px;">Employee Status <span class="red"> *</span></td>
        <td>
            <?php
...
<?php if($view_display =="display: block" || $print_display =="display: block"){  ?>
<table id="table1" >
    <tr>
        <td style="width: 150px;">Employee <span class="red"> *</span></td>
...
<input type="hidden" id="joined" value="<?php echo $_GET['join_date'];?>"> 
<table>
    <tr>
        ...
        <td colspan="2">Month&nbsp;&nbsp;
            ...
            ?>
            <select id="ddlmon" style="width: auto;" onchange="changemonth(<?php echo $emp; ?>)">
                <?php
                for ($i = 1; $i < 13; $i++) {
...
<script language="javascript" type="text/javascript">
    ...
    function changemonth(emp){ 
	  ...
	  var month_cur = today.getMonth();
	  var year_curr = today.getFullYear();
	  
	  var joined_date=$('#joined').val();
	  var date;
        var selection = $("#selection").val();
        var company = $("#dropCompany").val();
        var branch = $("#dropBranch").val();
        ...
        var year=$('#ddlyear').val();
		if(month<10){
		date= year +"-"+"0" + month +"-"+ dd;
		}else{
		date= year +"-" + month +"-"+ dd;
		}
		if(date<joined_date){
		alert("the payroll date should be after the joined date")
		 window.location = '?loc=payroll&show=true&emp_id='+emp+'&join_date='+joined_date+'&mon='+month_cur+'&year='+year_curr;
		 exit();
		}
