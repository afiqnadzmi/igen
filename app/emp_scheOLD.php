        ...
        <div id="container" class="tablediv" style="width:70%">
            <table style="padding-bottom: 20px;">
                ...
                <tr>
                    <td>Department</td>
                    <td>
                        <select id="dropDepartment" style="width: 250px;" onchange="showEmp(this.value)">
                            <option value="">--Please Select--</option>
                            <?php
                            if ($igen_companylist == "") {
                                if (isset($_GET["b"]) == true && $_GET["b"] != "") {
...
<script type="text/javascript">
    ...
    function showEmp(branch){
        var company = $("#dropCompany").val();
        window.location='?loc=emp_transfer&show=true&c='+company+'&b='+branch;
    }
    ...
    function getid(id, branch){
        var replacename = " ";
        var all_id = "";
		var dep_id ="";
       	$("input[type='checkbox']:checked").each(function (){ 
			all_id += $(this).val()+","; 
			dep_id += $(this).attr("alt")+",";
		}); 
		
        var newStr = all_id.substring(0, all_id.length-1);
        var replaceStr = newStr.replace(/on,/gi,replacename);
		dep_id=dep_id.substring(0, dep_id.length-1).replace(/on,/gi,replacename);
    ...
    function expand(obj,name,img){
	$("#dep").val(img);
        $('#'+name).toggle('slow');
        if($('#'+img).attr('src')=='images/But_DrillOut.png'){
            $('#'+img).attr('src','images/But_DrillIn.png');
        }else{
            $('#'+img).attr('src','images/But_DrillOut.png');
        }
    }
    ...
    function search_func(){
        var emp_name = document.getElementById("search_name").value;
        if(emp_name == ""){
