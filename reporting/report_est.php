...
$workbook->send('EST_Report(' . $month[$_GET["month"]] . ' ' . $_GET["year"] . ').xls');
$worksheet1->write(0, 0, 'EST Report For ' . $month[$_GET["month"]] . ' ' . $_GET["year"]);
$worksheet1->write(1, 0, "Employee");
$worksheet1->write(1, 1, "Employee EPF (RM)");
$worksheet1->write(1, 2, "Employee Socso (RM)");
$worksheet1->write(1, 3, "Employee PCB (RM)");
$worksheet1->write(1, 4, "Employer EPF (RM)");
$worksheet1->write(1, 5, "Employer Socso (RM)");
...
if ($chk_sql_result && mysql_num_rows($chk_sql_result) > 0) {
    ...
    if ($sql_result && mysql_num_rows($sql_result) > 0) {
        ...
        while ($newArray = mysql_fetch_array($sql_result)) {
            $emp_name = "EMP" . str_pad($newArray['emp_id'], 6, "0", STR_PAD_LEFT) . " " . $newArray['full_name'];
            $epf = $newArray['epf'];
            $socso = $newArray['socso'];
            $pcb = $newArray['pcb'];
            $e_epf = $newArray['employer_epf'];
            $e_socso = $newArray['employer_socso'];
            $worksheet1->write($i, 0, $emp_name);
            $worksheet1->write($i, 1, $epf);
            $worksheet1->write($i, 2, $socso);
            $worksheet1->write($i, 3, $pcb);
            $worksheet1->write($i, 4, $e_epf);
            $worksheet1->write($i, 5, $e_socso);
            $i++;
        }
    } else {
