...
$workbook->send('PCB_Report(' . $month[$_GET['month']] . " " . $_GET['year'] . ').xls');
$worksheet1->write(0, 0, "PCB Report For " . $month[$_GET['month']] . " " . $_GET['year']);
$worksheet1->write(1, 0, "Employee");
$worksheet1->write(1, 1, "I.C.");
$worksheet1->write(1, 2, "Salary (RM)");
$worksheet1->write(1, 3, "Bonus (RM)");
$worksheet1->write(1, 4, "Allowance (RM)");
$worksheet1->write(1, 5, "PCB (RM)");
...
if ($chk_sql_result && mysql_num_rows($chk_sql_result) > 0) {
    ...
    if ($sql_result && mysql_num_rows($sql_result) > 0) {
        while ($newArray = mysql_fetch_array($sql_result)) {
            ...
            $worksheet1->write($i, 5, $newArray['pcb']);
            $i++;
        }
    } else {
