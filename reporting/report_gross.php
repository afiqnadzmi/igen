...
$workbook->send('Gross_Pay_Report(' . $month[$_GET['month']] . " " . $_GET['year'] . ').xls');
$worksheet1->write(0, 0, 'Gross Pay Report For ' . $month[$_GET['month']] . " " . $_GET['year']);
...
if ($chk_sql_result && mysql_num_rows($chk_sql_result) > 0) {
    ...
    if ($sql_result && mysql_num_rows($sql_result) > 0) {
        while ($newArray = mysql_fetch_array($sql_result)) {
            ...
            $worksheet1->write($i, 9, $newArray['gross_pay']);
            $i++;
        }
    } else {
