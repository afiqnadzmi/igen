...
$workbook->send("Disciplinary_Report(" . $_GET['from'] . " to " . $_GET['to'] . ').xls');
$worksheet1->write(0, 0, 'Disciplinary Report For ' . $_GET['from'] . " to " . $_GET['to']);
$worksheet1->write(1, 0, "Employee");
$worksheet1->write(1, 1, "Note");
$worksheet1->write(1, 2, "Date");
...
if (mysql_num_rows($sql_result)) {
    while ($newArray = mysql_fetch_array($sql_result)) {
        ...
        $worksheet1->write($i, 1, $newArray['note']);
        $worksheet1->write($i, 2, $newArray['note_date']);
        $i++;
    }
} else {
