...
$workbook->send('Employer_Socso_Report(' . $month[$_GET['month']] . ' ' . $_GET['year'] . ').xls');
$worksheet1->write(0, 0, 'Employer Socso Report For ' . $month[$_GET['month']] . " " . $_GET['year']);
$worksheet1->write(1, 0, "Employee");
$worksheet1->write(1, 1, "Employer Socso (RM)");
...
if ($chk_sql_result && mysql_num_rows($chk_sql_result) > 0) {
    ...
    $fid = $chk_row["id"];
    $sql1 = "SELECT e.full_name,e.ic,e.epf_num,e.socso_num,p.* FROM payroll_report AS p
    ...
    if ($sql_result && mysql_num_rows($sql_result) > 0) {
        while ($newArray = mysql_fetch_array($sql_result)) {
            ...
            $worksheet1->write($i, 1, $newArray['employer_socso']);
            $i++;
        }
    } else {
