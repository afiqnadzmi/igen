...
$workbook->send('EPF_Report(' . $month[$_GET['month']] . " " . $_GET['year'] . ').xls');
$worksheet1->write(0, 0, 'EPF Report For ' . $month[$_GET['month']] . " " . $_GET['year']);
$worksheet1->write(1, 0, "Employee");
$worksheet1->write(1, 1, "I.C.");
$worksheet1->write(1, 2, "EPF Number");
$worksheet1->write(1, 3, "Basic Salary (RM)");
$worksheet1->write(1, 4, "Allowance & Bonus (RM)");
$worksheet1->write(1, 5, "EPF (RM)");
$worksheet1->write(1, 6, "Employer EPF (RM)");
...
if ($chk_sql_result && mysql_num_rows($chk_sql_result) > 0) {
    ...
    if ($sql_result && mysql_num_rows($sql_result) > 0) {
        while ($newArray = mysql_fetch_array($sql_result)) {
            ...
            $worksheet1->write($i, 2, $newArray['epf_num']);
            $worksheet1->write($i, 3, $newArray['basic_salary']);
            $worksheet1->write($i, 4, ((float) $newArray['allowance'] + (float) $newArray['bonus']));
            $worksheet1->write($i, 5, $newArray['epf']);
            $worksheet1->write($i, 6, $newArray['employer_epf']);
            $i++;
        }
    } else {
